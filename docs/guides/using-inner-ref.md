...
`@hello-pangea/dnd` does not support the dragging of `<svg>` elements. Wrap your `<svg>` in a `HTMLElement` such as `<span>` or `<div>` for great accessibility and cross browser support. See our [dragging SVGs guide](https://github.com/hello-pangea/dnd/blob/main/docs/guides/dragging-svgs.md) for more information.
